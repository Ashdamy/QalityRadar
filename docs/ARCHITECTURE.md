...
| Docker socket expuesto al worker = escape trivial a host | **El worker NO se ejecuta dentro de un contenedor**: corre en el host (proceso Celery directo en desarrollo, servicio systemd en el VPS) y habla con el demonio Docker local de forma nativa. Así nunca hace falta montar `/var/run/docker.sock` dentro de un contenedor, que es la vía clásica de escalada a root del host. El worker solo orquesta — nunca ejecuta código del usuario; lo aislado son los contenedores efímeros que lanza |
